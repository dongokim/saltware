...
/**  
 * @Class Name : CompleteCreditPortlet.java
 * @Description : 취득학점 Portlet
 * @
 * @author kevin
 ...
 */
public class CompleteCreditPortlet extends GenericServletPortlet
{
	private final Log   log = LogFactory.getLog(getClass());
	
	/**
	 * completeCreditPortlet을 초기화한다.
	 */
	...
	/**
	 * completeCreditPortlet의 화면을 출력한다.
	 *
	...
	public void doView(RenderRequest request, RenderResponse response)	throws PortletException, IOException 
	{
		try {
			...
			if( portletService != null ) {
				...
				throw new PortletException("There is no portletService about completeCredit");
			}
			
			request.setAttribute("results", results);
			
			// EXTRA DATA
			String moreTarget = Enview.getConfiguration().getString("portletService.completeCredit.moreTarget");
			if( moreTarget != null && moreTarget.length() > 0 ) {
				request.setAttribute("moreTarget", moreTarget);
			}
			String moreSrc = Enview.getConfiguration().getString("portletService.completeCredit.moreSrc");
	...
	/**
     * completeCreditPortlet의 헤더정보를 출력한다.
     *
	...
	/**
     * completeCreditPortlet의 편집화면을 출력한다.
     *
    ...
    /**
     * completeCreditPortlet의 도움말 화면을 출력한다.
     *
     * @see javax.portlet.GenericPortlet#doView
     */
	...
	/**
     * completeCreditPortlet의 액션을 처리한다.
	 *
