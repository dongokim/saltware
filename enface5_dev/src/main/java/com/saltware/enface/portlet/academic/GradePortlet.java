...
/**  
 * @Class Name : GradePortlet.java
 * @Description : 성적 Portlet
 * @
 * @author kevin
 ...
 */
public class GradePortlet extends GenericServletPortlet
{
	...
	public void doView(RenderRequest request, RenderResponse response)	throws PortletException, IOException 
	{
		try {
			...
			if( portletService != null ) {
				...
				throw new PortletException("There is no portletService about grade");
			}
			
			request.setAttribute("results", results);
			
			// EXTRA DATA
			String moreTarget = Enview.getConfiguration().getString("portletService.grade.moreTarget");
			if( moreTarget != null && moreTarget.length() > 0 ) {
				request.setAttribute("moreTarget", moreTarget);
			}
			String moreSrc = Enview.getConfiguration().getString("portletService.grade.moreSrc");
