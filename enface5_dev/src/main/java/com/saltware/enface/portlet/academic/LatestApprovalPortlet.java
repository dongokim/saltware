...
/**
 * @Class Name : LatestApprovalPortlet.java
 * @Description : 최근결재 Portlet @
 * @author kevin
 * @since 2014.03.04 12:58:385
 * @version 1.0
 * @see Copyright (C) by Saltware All right reserved.
 */
public class LatestApprovalPortlet extends GenericServletPortlet {
	private final Log log = LogFactory.getLog(getClass());

	/**
	 * latestApprovalPortlet을 초기화한다.
	 */
	...
	/**
	 * latestApprovalPortlet의 화면을 출력한다.
	 * 
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException {
		try {
			...
			if (portletService != null) {
				...
				throw new BaseException("There is no portletService about latestApproval");
			}

			request.setAttribute("results", results);

			// EXTRA DATA
			String moreTarget = Enview.getConfiguration().getString("portletService.latestApproval.moreTarget");
			if (moreTarget != null && moreTarget.length() > 0) {
				request.setAttribute("moreTarget", moreTarget);
			}
			String moreSrc = Enview.getConfiguration().getString("portletService.latestApproval.moreSrc");
	...
	/**
	 * latestApprovalPortlet의 헤더정보를 출력한다.
	 * 
	...
	/**
	 * latestApprovalPortlet의 편집화면을 출력한다.
	 * 
	...
	/**
	 * latestApprovalPortlet의 도움말 화면을 출력한다.
	 * 
	 * @see javax.portlet.GenericPortlet#doView
	 */
	...
	/**
	 * latestApprovalPortlet의 액션을 처리한다.
	 * 
