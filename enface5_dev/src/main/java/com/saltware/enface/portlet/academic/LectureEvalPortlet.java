...
/**  
 * @Class Name : LectureEvalPortlet.java
 * @Description : 강의평가 Portlet
 * @
 * @author kevin
 ...
 */
public class LectureEvalPortlet extends GenericServletPortlet
{
	private final Log   log = LogFactory.getLog(getClass());
	
    /**
	 * lectureEvalPortlet을 초기화한다.
	 */
	...
	public void doView(RenderRequest request, RenderResponse response)	throws PortletException, IOException 
	{
		try {
			...
			if( portletService != null ) {
				...
				throw new PortletException("There is no portletService about lectureEval");
			}
			
			request.setAttribute("results", results);
			
			// EXTRA DATA
			String moreTarget = Enview.getConfiguration().getString("portletService.lectureEval.moreTarget");
			if( moreTarget != null && moreTarget.length() > 0 ) {
				request.setAttribute("moreTarget", moreTarget);
			}
			String moreSrc = Enview.getConfiguration().getString("portletService.lectureEval.moreSrc");
	...
	/**
     * lectureEvalPortlet의 헤더정보를 출력한다.
     *
	...
	/**
     * lectureEvalPortlet의 편집화면을 출력한다.
     *
    ...
    /**
     * lectureEvalPortlet의 도움말 화면을 출력한다.
     *
     * @see javax.portlet.GenericPortlet#doView
     */
