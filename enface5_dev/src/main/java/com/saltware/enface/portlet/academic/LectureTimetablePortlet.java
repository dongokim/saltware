...
/**  
 * @Class Name : LectureTimetablePortlet.java
 * @Description : 강의시간표 Portlet
 * @
 * @author kevin
 ...
 */
public class LectureTimetablePortlet extends GenericServletPortlet
{
	private final Log   log = LogFactory.getLog(getClass());
	
	/**
	 * lectureTimetablePortlet을 초기화한다.
	 */
	...
	/**
	 * lectureTimetablePortlet의 화면을 출력한다.
	 *
	...
	public void doView(RenderRequest request, RenderResponse response)	throws PortletException, IOException 
	{
		try {
			...
			if( portletService != null ) {
				...
				throw new PortletException("There is no portletService about lectureTimetable");
			}
			
			request.setAttribute("results", results);
			
			// EXTRA DATA
			String moreTarget = Enview.getConfiguration().getString("portletService.lectureTimetable.moreTarget");
			if( moreTarget != null && moreTarget.length() > 0 ) {
				request.setAttribute("moreTarget", moreTarget);
			}
			String moreSrc = Enview.getConfiguration().getString("portletService.lectureTimetable.moreSrc");
	...
	/**
     * lectureTimetablePortlet의 헤더정보를 출력한다.
     *
	...
	/**
     * lectureTimetablePortlet의 편집화면을 출력한다.
     *
    ...
    /**
     * lectureTimetablePortlet의 도움말 화면을 출력한다.
     *
     * @see javax.portlet.GenericPortlet#doView
     */
	...
	/**
	 * 이 기능의 기본적인 행위는 ResourceRequest의 ResourceID로 RequestDispatcher.foward를 하는것이다.
	 * ResourceID가 없는 경우에는 아무런 행위를 안한다.
	 *
	 * @since 2.0
     */
	...
	/**
	 * 기본적인 기능은 주석이 '@ProcessEvent'로 된 메소드 중 주어진 event이름에 해당하는 메소드를 수행한다.
	 * 해당하는 메소드를 찾지 못하면 response에 현재 render parameter들을 저장한다.
	 * 이 메소드들은 모두 public으로 정의 되어야 한다.
	 * 
	...
	/**
     * lectureTimetablePortlet의 액션을 처리한다.
	 *
